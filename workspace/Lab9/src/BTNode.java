...
public class BTNode<E> {
	...
	public static BTNode<String> readFromFile(Scanner input) {
		...
		if (line.startsWith("?")) {
			String q = line.substring(1);
			root = new BTNode<String>(q, readFromFile(input),
					readFromFile(input));
		} else {
	...
	/**
	 * Write a tree with root to output using the data format
	 * 
	 * @param output
	...
	public static void writeToFile(PrintWriter output, BTNode<String> root) {
		if ((root.getData().substring(root.getData().length() - 1, root
				.getData().length())).equals("?")) {
