...
public class IntArrayBag implements Cloneable {
	 ...
	 *       cause the bag to fail with an arithmetic overflow.
	 **/
	public void add(int element) {
		if (manyItems == data.length) { // Ensure twice as much space as we
										// need.
	...
	/**
	 * Add new elements to this bag. If the new elements would take this bag
	 * beyond its current capacity, then the capacity is increased before adding
	 * the new elements.
	 * 
	 * @param elements
	...
	public void addMany(int... elements) {
		if (manyItems + elements.length > data.length) { // Ensure twice as much
															// space as we need.
	 ...
	 *                bag.
	 * @note An attempt to increase the capacity beyond Integer.MAX_VALUE will
	 *       cause an arithmetic overflow that will cause the bag to fail. Such
	...
	/**
	 * Accessor method to get the current capacity of this bag. The add method
	 ...
	 * @postcondition If target was found in the bag, then one copy of target
	...
	public boolean remove(int target) {
		...
		if (index == manyItems)
			...
			return false;
				...
				// So reduce manyItems by 1 and copy the last element onto
				// data[index].
			manyItems--;
	 ...
	 *       will cause an arithmetic overflow that will cause the bag to fail.
	 *       Such large collections should use a different bag implementation.
	 **/
	...
	/**
	 * This method checks if obj is a bag and it has exactly the same number of
	 * every element as the bag that activates the method
	 * 
	 * @param obj - the object to check
	 * 
	 * @return true if obj is a bag and has exactly the same number of every
	 *         element as the bag that activates the method
	 */
	public boolean equals(Object obj) {
		if (obj instanceof IntArrayBag) {
			IntArrayBag candidate = (IntArrayBag) obj;
			IntArrayBag candidate2 = candidate.clone();

			for (int i = 0; i < manyItems; i++) {
				int val = data[i];
				if (!candidate2.remove(val)) {
					return false;
				}
			}
			return true;
	...
	/**
	 * This method represents the elements that the bag contains. 
	 * 
	 * @return elements the bag contains in curly brackets ({})
